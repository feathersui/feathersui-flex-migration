...
class FlexListCollectionArrayCollectionTests extends Test {
	private static final TEXT_FILTER_ME = "__FILTER_ME__";

	private var _flexCollection:ArrayCollection;
	...
	public function setup():Void {
		...
		this._flexCollection = new ArrayCollection([this._a, this._b, this._c, this._d]);
		this._collection = new FlexListCollection(this._flexCollection);
	}
	...
	public function testResetList():Void {
		var newArrayCollection = new ArrayCollection([this._c, this._b, this._a]);
		var changeEvent = false;
		this._collection.addEventListener(Event.CHANGE, function(event:Event):Void {
			changeEvent = true;
		});
		...
		this._collection.list = newArrayCollection;
		...
		Assert.equals(newArrayCollection.length, this._collection.length, "Collection length must change after resetting collection with data of new size");
	}
	...
	public function testFilterFunction():Void {
		Assert.equals(this._flexCollection.source.length, this._collection.length, "Collection length must match source length if unfiltered");
		this._collection.filterFunction = filterFunction;
		Assert.notEquals(this._flexCollection.source.length, this._collection.length, "Collection length must not match source length if items are filtered");
		Assert.equals(this._flexCollection.source.length - 2, this._collection.length, "Collection length must account for filterFunction");
	...
	public function testSetFilterFunctionToNull():Void {
		...
		Assert.equals(this._flexCollection.source.length, this._collection.length,
	...
	public function testSetWithFilterFunctionAndNoMatch():Void {
		...
		this._collection.addEventListener(FlatCollectionEvent.REPLACE_ITEM, function(event:FlatCollectionEvent):Void {
			replaceItemEvent = true;
			indexFromEvent = event.index;
			trace(expectedIndex, event.index, event.addedItem, event.removedItem);
		});
		this._collection.addEventListener(FlatCollectionEvent.REMOVE_ITEM, function(event:FlatCollectionEvent):Void {
			removeItemEvent = true;
			indexFromEvent = event.index;
	...
	public function testSortCompareFunction():Void {
		...
		Assert.equals(this._flexCollection.source.length, this._collection.length, "Collection length must not change if sorted");
	...
	public function testAddAtWithSortCompareFunction():Void {
		...
		Assert.equals(newItem, this._collection.get(4), "Collection with sortCompareFunction and addAt() did not return correct item for unsorted index");
	}
